...
<?php 
/*
  This cgi only needs to return a part of html code.
...
if ( $_REQUEST['answer'] )
{
   ...
   # get the last valid index, since the index should be sorted
   # from small to big, the last valid index is also the max
   $max_index = $index[0];
   if (count($rating)>1)
   {   
   ...
   for ($i=0; $i<count($rating)-1; $i++) {
     ...
     # Since the smaller index means more popular dish, we give less difficulty
     # bonus for such question.
     # The weight is index/max_index.
     $bonus = ($difficulty_weight * ($index[$i]+1) / $max_index );
     # max_weight is the sum of all bonus for the questions
     $max_weight += $bonus;
     # difficulty_bonus only adds bonus for questions with 1 as answer
